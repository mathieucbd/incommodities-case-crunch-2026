"""LSTM model for electricity price forecasting.

Single-layer LSTM with LayerNorm + FC head. Uses a sliding-window lookback over
the input feature sequence, enabling temporal pattern capture across past hours.

The sklearn wrapper stores the last `lookback-1` rows of training data (the tail)
so that test-time predictions have full real context instead of zero-padding.
"""
...
class ElecLSTM(nn.Module):
    """Single-layer LSTM for electricity price regression.

    forward(x) takes (B, T, F) and returns (B,) — uses the last time step's
    hidden state after LayerNorm for the final linear projection.
    """
...
class LSTMRegressor(BaseEstimator, RegressorMixin):
    """Scikit-Learn wrapper for ElecLSTM.
    ...
    def __init__(
        ...
        self.models_: list[ElecLSTM] = []
        self.best_epoch_: int = 0
        self.best_epochs_: list[int] = []
        self._X_tail: np.ndarray | None = None

    def _train_single(self, X: np.ndarray, y: np.ndarray, seed: int) -> tuple[ElecLSTM, int]:
        ...
        ds_tr = TensorDataset(
            torch.from_numpy(seqs_tr).to(LSTM_DEVICE),
            torch.from_numpy(y_tr).to(LSTM_DEVICE),
        )
        loader = DataLoader(ds_tr, batch_size=self.batch_size, shuffle=True, drop_last=drop_last)

        seqs_va_t = torch.from_numpy(seqs_va).to(LSTM_DEVICE)
        y_va_t = torch.from_numpy(y_va).to(LSTM_DEVICE)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "LSTMRegressor":
        ...
        # Store tail for context-aware test predictions
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        seqs_t = torch.from_numpy(seqs).to(LSTM_DEVICE)
        preds_list = []
...
if __name__ == "__main__":
    ...
    model = LSTMRegressor(lookback=L, hidden_size=32, max_epochs=5, patience=5, n_seeds=2)
    model.fit(X, y)
    preds = model.predict(X[:100])
    assert preds.shape == (100,), f"Expected (100,), got {preds.shape}"

    # Simulate test scenario (tail prepend)
