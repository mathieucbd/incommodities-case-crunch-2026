...
def train_lightgbm(X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series, params: dict = None):
    ...
    if 'early_stopping_rounds' in params:
        ...
        try:
            ...
            fit_params['callbacks'] = [lgb.early_stopping(stopping_rounds=es_rounds, verbose=False)]
...
def train_xgboost(X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series, params: dict = None):
...
def train_catboost(X_train: pd.DataFrame, y_train: pd.Series, X_val: pd.DataFrame, y_val: pd.Series, params: dict = None):
...
def evaluate_model(model_name: str, model, X_test: pd.DataFrame, y_test: pd.Series) -> pd.Series:
...
if __name__ == "__main__":
    ...
    logger.info("Executing Train / Val / Test (Tree matrices bypassing Standard Scaler)...")
    logger.info("========================================")
