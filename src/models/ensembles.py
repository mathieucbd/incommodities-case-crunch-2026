...
from src.models.deep_learning import EPFMultivariateDNN, reshape_to_daily, train_pytorch_dnn
...
def run_ensemble():
    ...
    use_data_augmentation = config.get('model_settings', {}).get('dnn', {}).get('use_data_augmentation', True)
    
    # 1. Load & Preprocess Data for DE
    target_zone = "DE"
    logger.info("========================================")
    logger.info(f"Initiating Step 4: Quantile Regression Averaging (QRA) for {target_zone}...")
    ...
    model_lgb = train_lightgbm(X_train_raw, y_train_raw, X_val_raw, y_val_raw, params=lgb_params)
    ...
    model_xgb = train_xgboost(X_train_raw, y_train_raw, X_val_raw, y_val_raw, params=xgb_params)
    ...
    model_cat = train_catboost(X_train_raw, y_train_raw, X_val_raw, y_val_raw, params=cat_params)
    ...
    y_train_s_df, y_val_s_df, y_test_s_df, y_scaler = scale_data(y_train_raw.to_frame(), y_val_raw.to_frame(), y_test_raw.to_frame())
    ...
    model_dnn, device = train_pytorch_dnn(X_train_d, y_train_d, X_val_d, y_val_d, params=dnn_params)
    ...
    def align_preds(df_raw, preds_flattened):
        df_copy = pd.DataFrame({'Target': df_raw.values, 'Date': df_raw.index.date}, index=df_raw.index)
        valid_indices = []
