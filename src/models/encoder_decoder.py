"""Encoder-Decoder model for electricity price forecasting.

An asymmetric seq2seq architecture for single-step regression:
  - Encoder: multi-layer LSTM compresses the lookback sequence into a context vector
  - Decoder: lightweight MLP that combines the context vector with the most
    recent time step's features (the "query") to produce the scalar prediction.

The decoder concatenates [context, last_step_features] before the final MLP,
allowing the model to blend the compressed historical context with the
immediate market state. This is more efficient than full autoregressive
decoding since we only need one output step.
"""
...
if torch.backends.mps.is_available():
    ENCDEC_DEVICE = torch.device("mps")
elif torch.cuda.is_available():
    ENCDEC_DEVICE = torch.device("cuda")
else:
    ENCDEC_DEVICE = torch.device("cpu")
...
class ElecEncoderDecoder(nn.Module):
    """LSTM Encoder + MLP Decoder for electricity price regression.
    ...
    def __init__(
        ...
        # Encoder: LSTM compresses the full sequence into a context vector
        self.encoder = nn.LSTM(
            input_size, enc_hidden, enc_layers,
            batch_first=True,
            dropout=dropout if enc_layers > 1 else 0.0,
        )

        # Project encoder hidden → decoder hidden, then normalise
        self.ctx_proj = nn.Linear(enc_hidden, dec_hidden)
        self.ctx_norm = nn.LayerNorm(dec_hidden)

        # Decoder: MLP on [context || last_step_features]
        self.decoder = nn.Sequential(
            nn.Linear(dec_hidden + input_size, dec_hidden),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def _init_weights(self) -> None:
        for name, p in self.encoder.named_parameters():
            if "weight" in name:
                ...
                nn.init.zeros_(p)
        nn.init.xavier_normal_(self.ctx_proj.weight)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, T, F)
        _, (h_n, _) = self.encoder(x)     # h_n: (enc_layers, B, enc_hidden)
        h_last = h_n[-1]                   # last layer's hidden: (B, enc_hidden)

        # Context vector
        ctx = self.ctx_norm(self.ctx_proj(h_last))  # (B, dec_hidden)

        # Query: features at the last time step (current market state)
        query = x[:, -1, :]               # (B, input_size)

        combined = torch.cat([ctx, query], dim=-1)  # (B, dec_hidden + input_size)
        return self.decoder(combined).squeeze(-1)   # (B,)
...
class EncoderDecoderRegressor(BaseEstimator, RegressorMixin):
    """Scikit-Learn wrapper for ElecEncoderDecoder.
    ...
    def __init__(
        ...
        self.models_: list[ElecEncoderDecoder] = []
        self.best_epoch_: int = 0
        self.best_epochs_: list[int] = []
        self._X_tail: np.ndarray | None = None

    def _train_single(self, X: np.ndarray, y: np.ndarray, seed: int) -> tuple[ElecEncoderDecoder, int]:
        ...
        model = ElecEncoderDecoder(
            n_features, self.enc_hidden, self.dec_hidden, self.enc_layers, self.dropout
        ).to(ENCDEC_DEVICE)

        drop_last = len(seqs_tr) % self.batch_size < 2
        ds_tr = TensorDataset(
            torch.from_numpy(seqs_tr).to(ENCDEC_DEVICE),
            torch.from_numpy(y_tr).to(ENCDEC_DEVICE),
        )
        loader = DataLoader(ds_tr, batch_size=self.batch_size, shuffle=True, drop_last=drop_last)

        seqs_va_t = torch.from_numpy(seqs_va).to(ENCDEC_DEVICE)
        y_va_t = torch.from_numpy(y_va).to(ENCDEC_DEVICE)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "EncoderDecoderRegressor":
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        seqs_t = torch.from_numpy(seqs).to(ENCDEC_DEVICE)
        preds_list = []
...
if __name__ == "__main__":
    print(f"Device: {ENCDEC_DEVICE}")
    N, F, L = 500, 50, 48
    rng = np.random.default_rng(0)
    X = rng.standard_normal((N, F)).astype(np.float32)
    y = rng.standard_normal(N).astype(np.float32)

    model = EncoderDecoderRegressor(
        lookback=L, enc_hidden=32, dec_hidden=16, enc_layers=2,
