...
if __name__ == "__main__":
    ...
    X_val   = val_df[features];    y_val   = val_df[TARGET_COL]
    ...
    lear_preds = predict_lear(X_full_s, y_full, X_test.index, calibration_window_days=calibration_window)
    ...
    lgb_p = trees_c.get('lgb', {}).copy(); lgb_p['early_stopping_rounds'] = 50; lgb_p['random_state'] = seed
    xgb_p = trees_c.get('xgb', {}).copy(); xgb_p['early_stopping_rounds'] = 50; xgb_p['random_state'] = seed
    ...
    y_tr_s = y_tr_s_df[TARGET_COL]; y_va_s = y_va_s_df[TARGET_COL]
    ...
    qra_params = {k: v for k, v in qra_c.items() if k not in ('quantiles', 'alpha_winkler')}
    qra_params['random_state'] = seed
    q_models = train_qra(y_val.loc[common_val], val_base, quantiles=quantiles, params=qra_params)
    ...
    for h in range(24):
        ...
        y_h = y_h[valid]; lear_h = lear_h[valid]; qra_h = qra_h[valid]
        ...
        row = {'Hour': h, 'LEAR_MAE': mae_lear, 'QRA_MAE': mae_qra,
               'DM_stat': dm_stat, 'p_value': p_val, 'significant': sig.strip()}
        rows.append(row)

        p_str = f"{p_val:.4f}" if p_val is not None and not np.isnan(p_val) else "   N/A"
        d_str = f"{dm_stat:.4f}" if dm_stat is not None and not np.isnan(dm_stat) else "   N/A"
        logger.info(f"  {h:>3}    {mae_lear:>9.3f}   {mae_qra:>9.3f}   {d_str:>9}  {p_str:>9}  {sig}")
