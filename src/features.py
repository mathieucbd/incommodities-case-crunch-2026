...
def engineer_thermal_floor(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "fr_gas" in out.columns and "eu_emission" in out.columns:
        out["fr_thermal_floor"] = (out["fr_gas"] / 0.5) + (
            out["eu_emission"] * (0.202 / 0.5)
        )
    # UK Proxy
    if "uk_gas" in out.columns and "uk_emission" in out.columns:
        out["uk_thermal_floor"] = (out["uk_gas"] / 0.5) + (
            out["uk_emission"] * (0.202 / 0.5)
        )
    return out
...
def engineer_interconnectors(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    interconnector_cols = [c for c in out.columns if ("atc_" in c or "cost_" in c)]
    for col in interconnector_cols:
        # Add binary indicator mask for interconnectors with large missing blocks (>1000 rows)
        if out[col].isna().sum() > 1000:
            mask_name = f"is_{col}_online"
            out[mask_name] = out[col].notna().astype(int)
...
def engineer_calendar_effects(df: pd.DataFrame) -> pd.DataFrame:
    ...
    dates = dt_index.date
    out["is_fr_holiday"] = (
        pd.Series(dates, index=out.index).map(lambda x: x in fr_holidays).astype(int)
    )
    out["is_uk_holiday"] = (
        pd.Series(dates, index=out.index).map(lambda x: x in uk_holidays).astype(int)
    )
...
def engineer_weekly_lags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for target in ["fr_spot_la", "uk_spot_la"]:
        if target in out.columns:
            out[f"{target}_lag_168"] = out[target].shift(
                144
            )  # 144 (6 days) as features are already lagged by 24h
    return out
...
def engineer_scarcity(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all(
        ...
        out["fr_total_cap_f"] = (
            out["fr_nuclear_avcap_f"]
            + out["fr_gas_avcap_f"]
            + out["fr_hydro_res_f"]
            + out["fr_hydro_ror_f"]
        )
        out["fr_security_margin"] = out["fr_total_cap_f"] - out["fr_load_f"]
        out["fr_scarcity_ratio"] = out["fr_load_f"] / out["fr_total_cap_f"].replace(
            0, np.nan
        )
        out["fr_scarcity_ratio"] = (
            out["fr_scarcity_ratio"].replace([np.inf, -np.inf], np.nan).fillna(0)
        )
...
def engineer_regime_zscores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["fr_res_load", "uk_res_load"]:
        if col in out.columns:
            ...
            out[f"{col}_zscore_14d"] = (out[col] - rolling_mean) / rolling_std.replace(
                0, np.nan
            )
            out[f"{col}_zscore_14d"] = (
                out[f"{col}_zscore_14d"].replace([np.inf, -np.inf], np.nan).fillna(0)
            )
    return out
