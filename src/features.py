...
def apply_mad_filter(series: pd.Series, window: int | str = '24h', z: float = 3.0) -> pd.Series:
    ...
    mad = series.rolling(win_str).apply(lambda x: np.median(np.abs(x - np.median(x))), raw=True)
...
def create_lags(df: pd.DataFrame, columns: List[str], lags: List[int] = [24, 48, 168]) -> pd.DataFrame:
...
def greedy_feature_selector(target_zone: str, candidate_zones: List[str], raw_dir: str) -> Tuple[pd.DataFrame, List[str]]:
    """
    ...
    df_target['Spot_Price_Filtered'] = apply_mad_filter(df_target[TARGET_COL], window='24h', z=3.0)
    ...
    for neighbor in candidate_zones:
        ...
        try:
            ...
            df_neighbor['Spot_Price_Filtered'] = apply_mad_filter(df_neighbor[TARGET_COL], window='24h', z=3.0)
            ...
            df_slice = df_neighbor_lagged[neighbor_cols].rename(columns=lambda x: f"{neighbor}_{x}")
            ...
            if new_mae < best_mae:
                logger.info(f" -> ACCEPTED {neighbor}! MAE dropped: {best_mae:.3f} -> {new_mae:.3f}")
                best_mae = new_mae
                optimal_df = test_df
                current_features = test_features
                accepted_zones.append(neighbor)
            else:
                logger.info(f" -> REJECTED {neighbor}. MAE worsened/flat: {best_mae:.3f} vs {new_mae:.3f}")
...
if __name__ == "__main__":
    ...
    test_target = "DE"
    test_candidates = ["AT", "FR", "NL"]
    
    final_df, accepted = greedy_feature_selector(test_target, test_candidates, raw_directory)
