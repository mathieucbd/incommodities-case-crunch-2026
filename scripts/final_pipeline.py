"""Final pipeline v7 — 5-model ensemble with DNN + regime-based weights.

v7 changes vs v6:
  - Added DNN (PyTorch) as 5th ensemble model (349 deduped features, Huber loss)
  - FR: DNN [192, 96], dp=0.2 → low error corr with trees (0.70 with XGB, 0.80 with EN)
  - UK: DNN [768, 384, 192], dp=0.3 → 30-40% weight in all regimes
  - 5-model regime weights (5 regimes × 5 models = 25 params)
  - Expected gain: ~-1.0 RMSE on val vs 4-model
...
FR: Stationary target (EMA 240h) + CatBoost Optuna v2 + HBC
UK: Basis target (spot - merit_order_cost) + CatBoost d=8 + MAE loss + 150 features + HBC
...
Usage: cd "INCOMO 3" && python scripts/final_pipeline.py
"""
...
# ── Load data ─────────────────────────────────────────────────────────────
print("=" * 90)
print("  FINAL PIPELINE v7 — 5-model ensemble (CB + LGB + XGB + EN + DNN)")
print("=" * 90)
...
# ══════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 90)
print("  2. UK CatBoost — Basis modeling (150 confirmed features)")
print("=" * 90)
...
feat_uk_final = [f for f in feat_uk_confirmed if f in df_train.columns]
print(f"  UK features (confirmed): {len(feat_uk_final)}")

cb_uk = CatBoostRegressor(**UK_PARAMS)
cb_uk.fit(
    Pool(df_train.loc[df_train.index[valid_basis_tr], feat_uk_final],
...
print(f"  UK CatBoost: RMSE={rmse_uk_cb:.2f}, +HBC={rmse_uk_cb_hbc:.2f}, "
...
if HAS_LGB:
    ...
    lgb_uk.fit(
        df_train.loc[df_train.index[valid_basis_tr], feat_uk_final],
        y_basis_tr[valid_basis_tr],
...
# --- UK Elastic Net (alpha=1, l1_ratio=0.9) ---
uk_scaler = StandardScaler()
X_uk_tr_scaled = uk_scaler.fit_transform(
    np.nan_to_num(df_train.loc[df_train.index[valid_basis_tr], feat_uk_final].values, 0))
...
# Scale DNN features (shared scaler for FR/UK)
feat_dnn_final = [f for f in feat_dnn if f in df_train.columns]
dnn_scaler = StandardScaler()
X_dnn_tr = dnn_scaler.fit_transform(np.nan_to_num(df_train[feat_dnn_final].values, 0))
X_dnn_va = dnn_scaler.transform(np.nan_to_num(df_val[feat_dnn_final].values, 0))

# --- FR DNN [192, 96] ---
torch.manual_seed(42); np.random.seed(42)
dnn_fr = ElecDNN(len(feat_dnn_final), [192, 96], dropout=0.2)
dnn_fr, dnn_fr_epochs = train_dnn(
    dnn_fr, X_dnn_tr[fr_stat["valid_tr"]], fr_stat["y_dev_tr"][fr_stat["valid_tr"]].astype(np.float32),
    X_dnn_va[fr_stat["valid_va"]], fr_stat["y_dev_va"][fr_stat["valid_va"]].astype(np.float32))
preds_fr_dev_dnn = predict_dnn(dnn_fr, X_dnn_va)
...
dnn_uk, dnn_uk_epochs = train_dnn(
    dnn_uk, X_dnn_tr[valid_basis_tr], y_basis_tr[valid_basis_tr].astype(np.float32),
    X_dnn_va[valid_basis_va], y_basis_va[valid_basis_va].astype(np.float32))
preds_uk_dev_dnn = predict_dnn(dnn_uk, X_dnn_va)
...
# ── UK: Retrain (basis, no weights) ──────────────────────────────────────
uk_moc_full = df_full_train["uk_merit_order_cost"].values
uk_moc_test = df_test_pred["uk_merit_order_cost"].values
y_basis_full = train_fe["uk_spot"].values - uk_moc_full
valid_uk_full = np.isfinite(y_basis_full)

UK_FINAL = {**UK_PARAMS, "iterations": max(cb_uk.get_best_iteration() + 50, 500),
             "use_best_model": False}

print(f"  UK CatBoost (basis): training on {n_full} samples, {UK_FINAL['iterations']} iterations")
cb_uk_final = CatBoostRegressor(**UK_FINAL)
cb_uk_final.fit(
    Pool(df_full_train.loc[df_full_train.index[valid_uk_full], feat_uk_final],
...
# UK LightGBM on full data (needed by some regimes)
...
if HAS_LGB:
    ...
    if uk_lgb_needs:
        ...
        lgb_uk_final.fit(
            df_full_train.loc[df_full_train.index[valid_uk_full], feat_uk_final],
            y_basis_full[valid_uk_full],
        )
        preds_uk_test_lgb = uk_moc_test + lgb_uk_final.predict(df_test_pred[feat_uk_final])
        print(f"  UK LightGBM: retrained ({LGB_UK_FINAL['n_estimators']} iters)")

# UK XGBoost on full data (needed by some regimes)
...
if HAS_XGB:
    ...
    if uk_xgb_needs:
        ...
        print(f"  UK XGBoost: retrained ({XGB_UK_FINAL['n_estimators']} iters)")

# UK Elastic Net on full data
...
if uk_en_needs:
    ...
    X_uk_full_scaled = uk_scaler_full.fit_transform(
        np.nan_to_num(df_full_train.loc[df_full_train.index[valid_uk_full], feat_uk_final].values, 0))
    ...
    en_uk_final.fit(X_uk_full_scaled, y_basis_full[valid_uk_full])
    preds_uk_test_en = uk_moc_test + en_uk_final.predict(X_uk_test_scaled)
    print(f"  UK Elastic Net: retrained (n_nonzero={np.sum(en_uk_final.coef_ != 0)})")
...
if uk_dnn_needs:
    ...
    X_dnn_uk_full = dnn_scaler_uk_full.fit_transform(
        np.nan_to_num(df_full_train.loc[df_full_train.index[valid_uk_full], feat_dnn_final].values, 0))
    ...
    print(f"  UK DNN: retrained ({dnn_uk_epochs + 5} epochs)")
...
sub_a.to_csv("outputs/submission_v7_5model.csv", index=False)
sub_a.to_csv("outputs/submission.csv", index=False)
print(f"  submission_v7_5model.csv — {len(sub_a)} rows")
...
print(f"\n  Submission: outputs/submission_v7_5model.csv")

# Save results
results = {
    "fr": {
        "approach": "stationary_ema240h",
...
with open("outputs/final_pipeline_results.json", "w") as f:
