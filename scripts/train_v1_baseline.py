...
T1_FEATURES_FR = [
    ...
    "fr_solar_x_load", "fr_renewable_pen",
]
feat_fr = [f for f in feat_fr_28 + T1_FEATURES_FR if f in df_train.columns]
print(f"  Features: {len(feat_fr)} (28 base + {len(feat_fr) - 28} T1)")
...
# --- FR DNN [192, 96] — Huber_5 (default, best for ensemble diversity) ---
...
# --- UK DNN [768, 384, 192] — MSE loss (best for UK ensemble diversity) ---
torch.manual_seed(42); np.random.seed(42)
...
if fr_dnn_needs:
    ...
    print(f"  FR DNN: retrained ({retrain_epochs_fr} epochs, Huber_5)")
...
if uk_dnn_needs:
    ...
    print(f"  UK DNN: retrained full ({retrain_epochs_uk} epochs, MSE loss)")
