  ...
  MODE 2: INFERENCE (per round, < 3 min)
    ...
    Usage:
      python scripts/finals_inference_v2.py --inference \\
        ...
        --output   data/finals_round/y_test.csv
"""

import argparse, sys, json, time, warnings, pickle, os
import numpy as np
import pandas as pd
import torch
import yaml
...
# ══════════════════════════════════════════════════════════════════════════
# T2 HELPERS (identical to fulldata.py)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# HOLDOUT PIPELINE (identical to fulldata.py)
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# INFERENCE HELPER: predict all models on a feature DataFrame
# ══════════════════════════════════════════════════════════════════════════

def predict_on_features(df_fe, p, dnn_fr, dnn_uk):
    """Run all models on df_fe. Returns (fr_models_dict, uk_models_dict, hours, dow)."""
    feat_fr = p["feat_fr"]
    feat_uk = p["feat_uk"]
    feat_dnn_final = p["feat_dnn_final"]

    hours = df_fe["hour"].values
    dow = pd.to_datetime(df_fe["datetime_CET"]).dt.dayofweek.values
    rm_fr = df_fe["fr_stl_trend"].values
    uk_moc = df_fe["uk_merit_order_cost"].values

    # FR V9 models
    preds_fr_cb  = rm_fr + predict_tree(p["cb_fr"],  df_fe[feat_fr])
    preds_fr_lgb = rm_fr + predict_tree(p["lgb_fr"], df_fe[feat_fr])
    preds_fr_xgb = rm_fr + predict_tree(p["xgb_fr"], df_fe[feat_fr])
    preds_fr_en  = rm_fr + predict_elastic_net(p["en_fr"], p["en_fr_scaler"], df_fe[feat_fr].values)

    X_dnn_fr = p["dnn_scaler_fr"].transform(np.nan_to_num(df_fe[feat_dnn_final].values, 0))
    preds_fr_dnn = rm_fr + predict_dnn(dnn_fr, X_dnn_fr)

    fund_fr = build_fundamental_features(df_fe, "fr")
    X_ridge_fr = p["ridge_fr_scaler"].transform(np.nan_to_num(fund_fr.values.copy(), 0))
    preds_fr_ridge = p["ridge_fr"].predict(X_ridge_fr)

    # FR T2 + SR
    t2_fr_preds = {}
    for name_k, retrained in p["t2_fr_retrained"].items():
        feats = retrained["feats"]  # already filtered at pretrain time
        t2_fr_preds[name_k] = rm_fr + predict_t2(
            {k: v for k, v in retrained.items() if k != "feats"},
            df_fe[feats].values
        )

    fr_v9 = {"CB": preds_fr_cb, "LGB": preds_fr_lgb, "XGB": preds_fr_xgb,
             "EN": preds_fr_en, "DNN": preds_fr_dnn}
    v9_ens_fr = apply_regime_weights(fr_v9, hours, p["spring_v9_regime_fr"])

    X_sr_fr = np.column_stack([t2_fr_preds[k] for k in p["spring_fr_t2_names"]])
    preds_fr_sr = v9_ens_fr + p["spring_meta_fr"].predict(X_sr_fr)

    # UK V9 models
    preds_uk_cb  = uk_moc + predict_tree(p["cb_uk"],  df_fe[feat_uk])
    preds_uk_lgb = uk_moc + predict_tree(p["lgb_uk"], df_fe[feat_uk])
    preds_uk_xgb = uk_moc + predict_tree(p["xgb_uk"], df_fe[feat_uk])
    preds_uk_en  = uk_moc + predict_elastic_net(p["en_uk"], p["en_uk_scaler"], df_fe[feat_uk].values)

    X_dnn_uk = p["dnn_scaler_uk"].transform(np.nan_to_num(df_fe[feat_dnn_final].values, 0))
    preds_uk_dnn = uk_moc + predict_dnn(dnn_uk, X_dnn_uk)

    fund_uk = build_fundamental_features(df_fe, "uk")
    X_ridge_uk = p["ridge_uk_scaler"].transform(np.nan_to_num(fund_uk.values.copy(), 0))
    preds_uk_ridge = p["ridge_uk"].predict(X_ridge_uk)

    # UK XGB cluster
    xgb_cluster_basis = np.zeros(len(df_fe))
    for cname, c_hours in CLUSTER_SPLIT_SHIFTED_6H.items():
        hour_mask = np.isin(hours, c_hours)
        if hour_mask.sum() > 0:
            xgb_cluster_basis[hour_mask] = predict_tree(
                p["xgb_cluster_final"][cname],
                df_fe.loc[df_fe.index[hour_mask], feat_uk]
            )
    preds_uk_xgb_cluster = uk_moc + xgb_cluster_basis

    # UK T2 + SR
    t2_uk_preds = {}
    for name_k, retrained in p["t2_uk_retrained"].items():
        feats = retrained["feats"]  # already filtered at pretrain time
        t2_uk_preds[name_k] = uk_moc + predict_t2(
            {k: v for k, v in retrained.items() if k != "feats"},
            df_fe[feats].values
        )

    uk_v9 = {"CB": preds_uk_cb, "LGB": preds_uk_lgb, "XGB": preds_uk_xgb,
             "EN": preds_uk_en, "DNN": preds_uk_dnn}
    v9_ens_uk = apply_regime_weights(uk_v9, hours, p["spring_v9_regime_uk"])

    X_sr_uk_t2 = np.column_stack([t2_uk_preds[k] for k in p["spring_uk_t2_names"]])
    X_extra = np.column_stack([
        np.sin(2*np.pi*hours/24), np.cos(2*np.pi*hours/24),
        np.sin(2*np.pi*dow/7),    np.cos(2*np.pi*dow/7),
    ])
    X_sr_uk = np.hstack([X_sr_uk_t2, X_extra])
    preds_uk_sr = v9_ens_uk + p["spring_meta_uk"].predict(X_sr_uk)

    fr_models = {"CB": preds_fr_cb, "LGB": preds_fr_lgb, "XGB": preds_fr_xgb,
                 "EN": preds_fr_en, "DNN": preds_fr_dnn,
                 "RidgeF": preds_fr_ridge, "SR": preds_fr_sr}
    uk_models = {"CB": preds_uk_cb, "LGB": preds_uk_lgb, "XGB": preds_uk_xgb,
                 "EN": preds_uk_en, "DNN": preds_uk_dnn,
                 "RidgeF": preds_uk_ridge, "SR": preds_uk_sr,
                 "XGB_C": preds_uk_xgb_cluster}

    return fr_models, uk_models, hours, dow
...
def run_pretrain():
    print("=" * 90)
    print("  FINALS PRETRAIN — full data + double holdout + retrain + save pkl")
    print("=" * 90)
    t0 = time.time()
    ...
    print(f"  FULL DATA: {len(train_fe)} rows ({train_fe['datetime_CET'].min()} → {train_fe['datetime_CET'].max()})")
    print(f"  Loaded in {time.time()-t0:.0f}s")

    # ── Interaction feature ──
    if "fr_spot_la_roll_168h_mean" in train_fe.columns and "uk_price_per_mw_7d" in train_fe.columns:
        train_fe["X_fr_spot_la_roll_168h_mean_x_uk_price_per_mw_7d"] = (
            train_fe["fr_spot_la_roll_168h_mean"] * train_fe["uk_price_per_mw_7d"]
        )
    ...
    print(f"  STL done ({time.time()-t0:.0f}s)")
    ...
    print(f"  FR: {len(feat_fr)} feats, UK: {len(feat_uk)}, DNN: {len(feat_dnn_final)}")
    print(f"  T2 groups: FR={len(fr_groups)}, UK={len(uk_groups)}")

    # ── Holdouts ──
    # SPRING: Train →Sep'24, Val Oct-Dec'24
    ...
    spring = run_holdout("SPRING", df_train_spring, df_val_spring,
                         feat_fr, feat_uk, feat_dnn_final, fr_groups, uk_groups)

    # WINTER: Train →Jun'24, Val Nov'24-Feb'25
    ...
    winter = run_holdout("WINTER", df_train_winter, df_val_winter,
                         feat_fr, feat_uk, feat_dnn_final, fr_groups, uk_groups)

    # ── Averaged weights/HBC ──
    model_names_fr = ["CB", "LGB", "XGB", "EN", "DNN", "RidgeF", "SR"]
    model_names_uk = ["CB", "LGB", "XGB", "EN", "DNN", "RidgeF", "SR", "XGB_C"]
    avg_fr_weights = {}
    avg_uk_weights = {}
    for regime in REGIMES:
        avg_fr_weights[regime] = {m: (spring["fr_regime_weights"].get(regime, {}).get(m, 0) +
                                      winter["fr_regime_weights"].get(regime, {}).get(m, 0)) / 2
                                  for m in model_names_fr}
        avg_uk_weights[regime] = {m: (spring["uk_regime_weights"].get(regime, {}).get(m, 0) +
                                      winter["uk_regime_weights"].get(regime, {}).get(m, 0)) / 2
    ...
    print(f"\n  SPRING: FR={spring['val_metrics']['fr_hbc']:.4f} UK={spring['val_metrics']['uk_hbc']:.4f}")
    print(f"  WINTER: FR={winter['val_metrics']['fr_hbc']:.4f} UK={winter['val_metrics']['uk_hbc']:.4f}")

    # ══════════════════════════════════════════════════════════════════════
    # RETRAIN ON FULL DATA
    # ══════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 90)
    print("  RETRAIN on full data")
    print("=" * 90)
    t_retrain = time.time()

    # Coherent STL for full train (no separate test here)
    stl_retrain = STL(spot_la_series, period=168, seasonal=13)
    result_retrain = stl_retrain.fit()
    rm_fr_full = result_retrain.trend.values
    spot_fr_full = train_fe["fr_spot"].values
    y_dev_fr_full = spot_fr_full - rm_fr_full
    ...
    t2_fr_retrained = {}
    for name_k, t2_info in spring["t2_fr"].items():
        feats = t2_info["feats"]
        available = [f for f in feats if f in train_fe.columns]
        retrained = retrain_t2(t2_info["algo"],
                               train_fe[available].values[valid_fr_full],
                               y_dev_fr_full[valid_fr_full], t2_info["info"])
        retrained["feats"] = available
        t2_fr_retrained[name_k] = retrained
    ...
    print(f"  UK CB retrained ({si['cb_uk']} iter)")
    ...
    print(f"  UK LGB retrained ({si['lgb_uk']} iter)")
    ...
    print(f"  UK XGB retrained ({si['xgb_uk']} iter)")
    ...
    for cname, c_hours in CLUSTER_SPLIT_SHIFTED_6H.items():
        ...
        xgb_cluster_final[cname] = retrain_tree("xgboost", XGB_UK_CLUSTER_P,
            train_fe.loc[train_fe.index[c_mask_full], feat_uk], y_basis_full[c_mask_full], best_iter)
    print(f"  UK XGB cluster retrained (4 clusters)")

    print(f"  UK T2 retraining {len(spring['t2_uk'])} models...")
    t2_uk_retrained = {}
    for name_k, t2_info in spring["t2_uk"].items():
        feats = t2_info["feats"]
        available = [f for f in feats if f in train_fe.columns]
        retrained = retrain_t2(t2_info["algo"],
                               train_fe[available].values[valid_uk_full],
                               y_basis_full[valid_uk_full], t2_info["info"])
        retrained["feats"] = available
        t2_uk_retrained[name_k] = retrained
    ...
    pretrain_data = {
        ...
        "feat_fr": feat_fr,
        "feat_uk": feat_uk,
        "feat_dnn_final": feat_dnn_final,
        "fr_groups": fr_groups,
        "uk_groups": uk_groups,

        # Clipping
        "clip_fr_low": fr_q_low, "clip_fr_high": fr_q_high,
        "clip_uk_low": uk_q_low, "clip_uk_high": uk_q_high,

        # Spring weights/HBC/meta
        "spring_fr_weights": spring["fr_regime_weights"],
        "spring_uk_weights": spring["uk_regime_weights"],
        "spring_hbc_fr": spring["hbc_fr"],
        "spring_hbc_uk": spring["hbc_uk"],
        "spring_v9_regime_fr": spring["v9_regime_fr"],
        "spring_v9_regime_uk": spring["v9_regime_uk"],
        "spring_meta_fr": spring["meta_fr"],
        "spring_meta_uk": spring["meta_uk"],
        "spring_fr_t2_names": spring["fr_t2_names"],
        "spring_uk_t2_names": spring["uk_t2_names"],

        # Winter weights/HBC
        "winter_fr_weights": winter["fr_regime_weights"],
        "winter_uk_weights": winter["uk_regime_weights"],
        "winter_hbc_fr": winter["hbc_fr"],
        "winter_hbc_uk": winter["hbc_uk"],

        # Averaged weights/HBC
        "avg_fr_weights": avg_fr_weights,
        "avg_uk_weights": avg_uk_weights,
        "avg_hbc_fr": avg_hbc_fr,
        "avg_hbc_uk": avg_hbc_uk,

        # STL context (fr_spot_la series for inference continuity)
        "train_fr_spot_la": spot_la_series.reset_index(drop=True),
        ...
        "xgb_fr": xgb_fr_final,
        "en_fr": en_fr_final,
        "en_fr_scaler": en_fr_scaler,
        "dnn_fr_state": {k: v.cpu() for k, v in dnn_fr_final.state_dict().items()},
        "dnn_fr_arch": {"input_size": len(feat_dnn_final), "hidden": [192, 96], "dropout": 0.2},
        "dnn_scaler_fr": dnn_scaler_full_fr,
        "ridge_fr": ridge_fr_full,
        "ridge_fr_scaler": scaler_rf_full,
        "t2_fr_retrained": t2_fr_retrained,
        ...
        "xgb_uk": xgb_uk_final,
        "en_uk": en_uk_final,
        "en_uk_scaler": en_uk_scaler,
        "dnn_uk_state": {k: v.cpu() for k, v in dnn_uk_final.state_dict().items()},
        "dnn_uk_arch": {"input_size": len(feat_dnn_final), "hidden": [768, 384, 192], "dropout": 0.3},
        "dnn_scaler_uk": dnn_scaler_full_uk,
        "ridge_uk": ridge_uk_full,
        "ridge_uk_scaler": scaler_ruk_full,
        "xgb_cluster_final": xgb_cluster_final,
        "t2_uk_retrained": t2_uk_retrained,
    }

    with open(PRETRAIN_PATH, "wb") as f:
        pickle.dump(pretrain_data, f, protocol=4)

    size_mb = os.path.getsize(PRETRAIN_PATH) / 1e6
    print(f"  Saved {PRETRAIN_PATH} ({size_mb:.0f} MB)")
    print(f"\n  Total pretrain time: {time.time()-t0:.0f}s")
...
def run_inference(args):
    ...
    with open(PRETRAIN_PATH, "rb") as f:
        p = pickle.load(f)
    print(f"  Pretrain loaded ({time.time()-t0:.1f}s)")

    # ── Reconstruct DNN models ──
    dnn_fr = ElecDNN(p["dnn_fr_arch"]["input_size"], p["dnn_fr_arch"]["hidden"], p["dnn_fr_arch"]["dropout"])
    dnn_fr.load_state_dict(p["dnn_fr_state"])
    dnn_fr.eval()

    dnn_uk = ElecDNN(p["dnn_uk_arch"]["input_size"], p["dnn_uk_arch"]["hidden"], p["dnn_uk_arch"]["dropout"])
    dnn_uk.load_state_dict(p["dnn_uk_state"])
    dnn_uk.eval()

    # ── Load round data ──
    x_val_df  = pd.read_csv(args.x_val)
    x_test_df = pd.read_csv(args.x_test)
    y_val_df  = pd.read_csv(args.y_val)
    print(f"  Round data: x_val={len(x_val_df)}h, x_test={len(x_test_df)}h, y_val={len(y_val_df)}h")

    # ── Build features on concat(x_val + x_test) ──
    x_combined = pd.concat([x_val_df, x_test_df], ignore_index=True)
    combined_fe = build_features(x_combined, config)
    n_val = len(x_val_df)

    # Interaction feature
    if "fr_spot_la_roll_168h_mean" in combined_fe.columns and "uk_price_per_mw_7d" in combined_fe.columns:
        combined_fe["X_fr_spot_la_roll_168h_mean_x_uk_price_per_mw_7d"] = (
            combined_fe["fr_spot_la_roll_168h_mean"] * combined_fe["uk_price_per_mw_7d"]
        )

    # ── STL: concat train history + validation + test for continuity ──
    train_spot_la = p["train_fr_spot_la"]  # Series, reset index
    combined_spot_la = combined_fe["fr_spot_la"].ffill().bfill().reset_index(drop=True)
    full_spot_la_infer = pd.concat([train_spot_la, combined_spot_la], ignore_index=True)

    stl_infer = STL(full_spot_la_infer, period=168, seasonal=13)
    result_infer = stl_infer.fit()
    n_train = len(train_spot_la)
    combined_trend    = result_infer.trend.values[n_train:]
    combined_seasonal = result_infer.seasonal.values[n_train:]

    combined_fe = combined_fe.reset_index(drop=True)
    combined_fe["fr_stl_trend"]    = combined_trend
    combined_fe["fr_stl_seasonal"] = combined_seasonal

    val_fe  = combined_fe.iloc[:n_val].copy()
    test_fe = combined_fe.iloc[n_val:].copy()
    print(f"  Features built, STL done ({time.time()-t0:.1f}s)")

    # ── Predict on validation (for HBC recalibration) ──
    print(f"  Predicting on validation ({n_val}h)...")
    val_fr_models, val_uk_models, val_hours, _ = predict_on_features(val_fe, p, dnn_fr, dnn_uk)

    preds_val_fr = apply_regime_weights(val_fr_models, val_hours, p["avg_fr_weights"])
    preds_val_uk = apply_regime_weights(val_uk_models, val_hours, p["avg_uk_weights"])

    # Recalibrate HBC from validation residuals
    actuals_fr = y_val_df["fr_spot"].values
    actuals_uk = y_val_df["uk_spot"].values
    valid_fr_val = np.isfinite(actuals_fr)
    valid_uk_val = np.isfinite(actuals_uk)

    hbc_recal_fr, rmse_val_fr = compute_hbc(preds_val_fr[valid_fr_val], actuals_fr[valid_fr_val], val_hours[valid_fr_val])
    hbc_recal_uk, rmse_val_uk = compute_hbc(preds_val_uk[valid_uk_val], actuals_uk[valid_uk_val], val_hours[valid_uk_val])
    print(f"  Val RMSE (pre-HBC): FR={rmse_val_fr:.3f} UK={rmse_val_uk:.3f}")

    # ── Predict on test (24h) ──
    print(f"  Predicting on test ({len(test_fe)}h)...")
    test_fr_models, test_uk_models, test_hours, _ = predict_on_features(test_fe, p, dnn_fr, dnn_uk)

    # Spring ensemble
    preds_spring_fr = apply_regime_weights(test_fr_models, test_hours, p["spring_fr_weights"])
    preds_spring_uk = apply_regime_weights(test_uk_models, test_hours, p["spring_uk_weights"])

    # Winter ensemble
    preds_winter_fr = apply_regime_weights(test_fr_models, test_hours, p["winter_fr_weights"])
    preds_winter_uk = apply_regime_weights(test_uk_models, test_hours, p["winter_uk_weights"])

    # Average spring + winter (same as attack_averaged)
    preds_fr = (preds_spring_fr + preds_winter_fr) / 2
    preds_uk = (preds_spring_uk + preds_winter_uk) / 2

    # Apply recalibrated HBC
    preds_fr_hbc = preds_fr + np.array([hbc_recal_fr.get(h, 0) for h in test_hours])
    preds_uk_hbc = preds_uk + np.array([hbc_recal_uk.get(h, 0) for h in test_hours])

    # Clip
    preds_fr_final = np.clip(preds_fr_hbc, p["clip_fr_low"], p["clip_fr_high"])
    preds_uk_final = np.clip(preds_uk_hbc, p["clip_uk_low"], p["clip_uk_high"])

    # ── Output ──
    # Recover original test IDs
    x_test_reread = pd.read_csv(args.x_test)
    if "id" in x_test_reread.columns:
        ids = x_test_reread["id"].values
    else:
        ids = test_fe.index.values

    result = pd.DataFrame({
        "id": ids,
        "fr_spot": preds_fr_final,
        "uk_spot": preds_uk_final,
    })
    result.to_csv(args.output, index=False)
    print(f"  Saved {args.output}")
    print(f"  FR: mean={preds_fr_final.mean():.1f}  UK: mean={preds_uk_final.mean():.1f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrain",   action="store_true", help="Run pretrain (MODE 1)")
    parser.add_argument("--inference",  action="store_true", help="Run inference (MODE 2)")
    parser.add_argument("--x_test",  default="data/finals_round/x_test.csv")
    parser.add_argument("--x_val",   default="data/finals_round/x_validation.csv")
    parser.add_argument("--y_val",   default="data/finals_round/y_validation.csv")
    parser.add_argument("--output",  default="data/finals_round/y_test.csv")
    ...
    if args.pretrain:
        ...
        print("Specify --pretrain or --inference")
        parser.print_help()
