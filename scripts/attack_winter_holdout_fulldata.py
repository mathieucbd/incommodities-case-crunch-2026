...
Usage: cd "INCOMO 3" && python -u scripts/attack_winter_holdout_fulldata.py
"""
...
def run_holdout(name, df_train_h, df_val_h, feat_fr, feat_uk, feat_dnn_final,
    ...
    X_rf_tr = scaler_rf.fit_transform(np.nan_to_num(fund_fr_tr.values, 0))
    X_rf_va = scaler_rf.transform(np.nan_to_num(fund_fr_va.values, 0))
    ...
    X_ruk_tr = scaler_ruk.fit_transform(np.nan_to_num(fund_uk_tr.values, 0))
    X_ruk_va = scaler_ruk.transform(np.nan_to_num(fund_uk_va.values, 0))
