"""Final pipeline v10 — Add HybridLinearMLP as 6th model.

v10 changes vs v9:
  - ADD: HybridMLPRegressor (arXiv:2601.02856) as 6th model in ensemble
    Architecture: linear shortcut + 3-layer MLP, AdamW + HuberLoss
    Uses same feat_dnn features + StandardScaler as the existing DNN
  - All other logic, hyperparameters and fixes from v9 are preserved verbatim
...
Usage: cd "INCOMO 3" && python scripts/final_pipeline_v10.py
"""
...
# ══════════════════════════════════════════════════════════════════════════
print("=" * 90)
print("  FINAL PIPELINE v10 — HybridLinearMLP as 6th model (arXiv:2601.02856)")
print("=" * 90)
...
print(
    f"  Train FR (full): {len(df_train)}, Train UK (12m): {len(df_train_uk)}, "
    f"Val: {len(df_val)}, Test: {len(test_fe)}"
)
print(f"  DNN device: {DNN_DEVICE}")
print(f"  HybridMLP device: {MLP_DEVICE}")
...
print(
    f"  UK features: {len(feat_uk_confirmed)} (v7) → {len(feat_uk_v8)} (v8, +{len(V8_NEW_UK)} new)"
)

# DNN / HybridMLP: All numeric features (deduped at corr > 0.99)
...
print(f"  DNN/HybridMLP features: {len(feat_dnn)} (after 0.99 corr dedup)")
...
feat_fr = [
    f for f in feat_fr_28 if f in df_train.columns
]  # v7: 28 features (no v8 additions)
...
print(
    f"  CatBoost FR: RMSE={rmse_fr_cb:.2f}, +HBC={rmse_fr_cb_hbc:.2f}, iter={cb_fr.best_iteration}"
)
...
feat_uk_final = [
    f for f in feat_uk_confirmed if f in df_train_uk.columns
]  # v7: 150 features (no v8 additions)
...
print(
    f"  UK CatBoost (12m): RMSE={rmse_uk_cb:.2f}, +HBC={rmse_uk_cb_hbc:.2f}, iter={cb_uk.best_iteration}"
)
...
if HAS_LGB:
    ...
    print(
        f"  LGB FR: RMSE={rmse_fr_lgb:.2f}, +HBC={rmse_fr_lgb_hbc:.2f}, iter={lgb_fr.best_iteration}"
    )
    ...
    print(
        f"  LGB UK: RMSE={rmse_uk_lgb:.2f}, +HBC={rmse_uk_lgb_hbc:.2f}, iter={lgb_uk.best_iteration}"
    )
...
if HAS_XGB:
    ...
    print(
        f"  XGB FR: RMSE={rmse_fr_xgb:.2f}, +HBC={rmse_fr_xgb_hbc:.2f}, iter={xgb_fr.best_iteration}"
    )
    ...
    print(
        f"  XGB UK: RMSE={rmse_uk_xgb:.2f}, +HBC={rmse_uk_xgb_hbc:.2f}, iter={xgb_uk.best_iteration}"
    )
...
print(
    f"  EN FR: RMSE={rmse_fr_en:.2f}, +HBC={rmse_fr_en_hbc:.2f}, "
...
print(
    f"  EN UK: RMSE={rmse_uk_en:.2f}, +HBC={rmse_uk_en_hbc:.2f}, "
...
print(
    f"  DNN FR: RMSE={rmse_fr_dnn:.2f}, +HBC={rmse_fr_dnn_hbc:.2f}, ep={dnn_fr_epochs}"
)
...
print(
    f"  DNN UK: RMSE={rmse_uk_dnn:.2f}, +HBC={rmse_uk_dnn_hbc:.2f}, ep={dnn_uk_epochs}"
)


# ══════════════════════════════════════════════════════════════════════════
# 3e. HybridLinearMLP models (same feat_dnn features, sklearn wrapper)
# ══════════════════════════════════════════════════════════════════════════
print("\n" + "=" * 90)
print(f"  3e. HybridLinearMLP ({len(feat_dnn_final)} features, arXiv:2601.02856)")
print("=" * 90)

# Reuse scaled arrays from DNN step (same scaler, same feature list)
# FR HybridMLP — [256, 128, 64] pathway (optimized: Huber10, cosine warmup, GELU, 3-seed)
...
print(
    ...
    f"best_ep={hmlp_fr.best_epoch_} [{'/'.join(str(e) for e in hmlp_fr.best_epochs_)}]"
)

# UK HybridMLP — [512, 256, 128] pathway (optimized: MSE, cosine warmup, GELU, 3-seed)
...
print(
    ...
    f"best_ep={hmlp_uk.best_epoch_} [{'/'.join(str(e) for e in hmlp_uk.best_epochs_)}]"
)
...
# UK: all models including HybridMLP
...
if HAS_LGB:
    ...
    if fr_lgb_needs:
        ...
        preds_fr_test_lgb = rm_fr_test + predict_tree(
            lgb_fr_final, df_test_pred[feat_fr]
        )
...
if HAS_XGB:
    ...
    if fr_xgb_needs:
        ...
        preds_fr_test_xgb = rm_fr_test + predict_tree(
            xgb_fr_final, df_test_pred[feat_fr]
        )
...
if fr_dnn_needs:
    ...
    X_dnn_full = dnn_scaler_full.fit_transform(
        np.nan_to_num(
            df_full_train.loc[
                df_full_train.index[valid_fr_full], feat_dnn_final
            ].values,
            0,
        )
    )
    ...
    dnn_fr_final, _ = train_dnn(
        ...
        max_epochs=retrain_epochs_fr,
        patience=retrain_epochs_fr,
    )
...
if fr_hmlp_needs:
    ...
    hmlp_fr_final = HybridMLPRegressor(
        ...
        weight_decay=1e-4,
        batch_size=128,
        max_epochs=hmlp_fr.best_epoch_ + 5,
        patience=hmlp_fr.best_epoch_ + 5,  # effectively no early stopping
        val_fraction=0.05,
        loss="huber",
        huber_delta=10.0,
        warmup_epochs=5,
...
if preds_fr_test_hmlp is not None:
    fr_test_models["HMLP"] = preds_fr_test_hmlp
preds_fr_test = apply_regime_weights(fr_test_models, hours_test, fr_regime_weights)

preds_fr_test_hbc = preds_fr_test + np.array(
    [hbc_fr_final.get(h, 0) for h in hours_test]
)
...
print(
    f"  FR test predictions (std HBC): min={preds_fr_test_hbc.min():.1f}, "
...
if HAS_LGB:
    ...
    if uk_lgb_needs:
        ...
        preds_uk_test_lgb = uk_moc_test + predict_tree(
            lgb_uk_final, df_test_pred[feat_uk_final]
        )
...
if HAS_XGB:
    ...
    if uk_xgb_needs:
        ...
        preds_uk_test_xgb = uk_moc_test + predict_tree(
            xgb_uk_final, df_test_pred[feat_uk_final]
        )
...
if uk_en_needs:
    en_uk_final, en_uk_scaler_full = retrain_elastic_net(
        df_full_train_uk.loc[
            df_full_train_uk.index[valid_uk_full], feat_uk_final
        ].values,
        y_basis_full[valid_uk_full],
        alpha=1.0,
        l1_ratio=0.9,
    ...
    print(
        f"  UK Elastic Net: retrained full (n_nonzero={np.sum(en_uk_final.coef_ != 0)})"
    )
...
if uk_dnn_needs:
    ...
    X_dnn_uk_full = dnn_scaler_uk_full.fit_transform(
        np.nan_to_num(
            df_full_train_uk.loc[
                df_full_train_uk.index[valid_uk_full], feat_dnn_final
            ].values,
            0,
        )
    )
    ...
    dnn_uk_final, _ = train_dnn(
        ...
        max_epochs=retrain_epochs_uk,
        patience=retrain_epochs_uk,
    )
...
if uk_hmlp_needs:
    ...
    hmlp_uk_final = HybridMLPRegressor(
        ...
        weight_decay=1e-4,
        batch_size=128,
        max_epochs=hmlp_uk.best_epoch_ + 5,
        patience=hmlp_uk.best_epoch_ + 5,  # effectively no early stopping
        val_fraction=0.05,
        loss="mse",
        warmup_epochs=5,
        grad_clip=1.0,
    ...
    print(f"  UK HybridMLP: retrained full ({hmlp_uk.best_epoch_ + 5} epochs)")
...
if preds_uk_test_hmlp is not None:
    uk_test_models["HMLP"] = preds_uk_test_hmlp
preds_uk_test = apply_regime_weights(uk_test_models, hours_test, uk_regime_weights)

preds_uk_test_hbc = preds_uk_test + np.array(
    [hbc_uk_final.get(h, 0) for h in hours_test]
)
...
print(
    f"  UK test predictions (std HBC): min={preds_uk_test_hbc.min():.1f}, "
...
sub_a.to_csv("outputs/submission_v10.csv", index=False)
sub_a.to_csv("outputs/submission.csv", index=False)
print(f"  submission_v10.csv — {len(sub_a)} rows")
...
print(
    f"    FR HybridMLP:      RMSE={rmse_fr_hmlp:.2f}  +HBC={rmse_fr_hmlp_hbc:.2f}  (ep={hmlp_fr.best_epoch_})"
)
...
print(
    f"    UK CatBoost:       RMSE={rmse_uk_cb:.2f}  +HBC={rmse_uk_cb_hbc:.2f}  ({uk_approach})"
)
if preds_uk_lgb is not None:
...
print(
    f"    UK HybridMLP:      RMSE={rmse_uk_hmlp:.2f}  +HBC={rmse_uk_hmlp_hbc:.2f}  (ep={hmlp_uk.best_epoch_})"
)
...
print(f"\n  Submission: outputs/submission_v10.csv")

results = {
    "fr": {
        "approach": "stationary_ema240h_v10_hybrid_mlp",
        "catboost_rmse": rmse_fr_cb,
        "catboost_rmse_hbc": rmse_fr_cb_hbc,
        "lightgbm_rmse": rmse_fr_lgb if preds_fr_lgb is not None else None,
        "hybrid_mlp_rmse": rmse_fr_hmlp,
        "hybrid_mlp_rmse_hbc": rmse_fr_hmlp_hbc,
        "hybrid_mlp_best_epoch": hmlp_fr.best_epoch_,
        "regime_ensemble_rmse": float(rmse_fr_ens),
        "final_rmse_hbc": rmse_fr_final,
        "regime_weights": {
            k: {nm: float(v) for nm, v in w.items()}
            for k, w in fr_regime_weights.items()
        },
        "features": feat_fr,
        "params": {
            k: v
    ...
    "uk": {
        ...
        "hybrid_mlp_rmse": rmse_uk_hmlp,
        "hybrid_mlp_rmse_hbc": rmse_uk_hmlp_hbc,
        "hybrid_mlp_best_epoch": hmlp_uk.best_epoch_,
        "regime_ensemble_rmse": float(rmse_uk_ens),
        "final_rmse_hbc": rmse_uk_final,
        "regime_weights": {
            k: {nm: float(v) for nm, v in w.items()}
            for k, w in uk_regime_weights.items()
        },
        "n_features": len(feat_uk_final),
        "params": {
            k: v
            for k, v in UK_PARAMS.items()
            if k not in ["verbose", "allow_writing_files"]
        },
    },
...
with open("outputs/final_pipeline_v10_results.json", "w") as f:
