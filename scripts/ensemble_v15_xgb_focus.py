#!/usr/bin/env python
"""XGBoost-Focused Ensemble: 9 XGBoost models with varying random seeds and depths.

Since V14 showed XGB depth=7 as top performer, concentrate ensemble diversity
on XGBoost with multiple random seeds (3 depths x 3 seeds = 9 models).
"""
...
def main():
    print("\n" + "="*80)
    print("  XGBoost-Focused Ensemble (V15): 9 XGB Models")
    print("="*80)
    ...
    print(f"\n  Training 9 XGBoost models (3 depths x 3 seeds)...")
    ...
    for depth in depths:
        for seed in seeds:
            xgb = XGBRegressor(max_depth=depth, learning_rate=0.05, n_estimators=400,
                               subsample=0.8, colsample_bytree=0.8,
                               random_state=seed, n_jobs=-1, verbosity=0)
            xgb.fit(X_train_fr, y_train_fr_dev)
            models_fr[f"XGB_d{depth}_s{seed}"] = xgb
    ...
    for depth in depths:
        for seed in seeds:
            ...
            models_uk[f"XGB_d{depth}_s{seed}"] = xgb
